...
export function createJourney() {
  ...
  let step=1,started=0,complete=false,txHash='',ended=0,terminal='';
  const clockKey=()=> 'proofsettle.clock.'+txHash.toLowerCase();
  function saveClock(){try{localStorage.setItem(clockKey(),JSON.stringify({started,ended,terminal}));}catch{}}
  function renderClock(){if(!started){$('#order-elapsed').textContent='Checking order time…';return;}const secs=Math.max(0,Math.floor(((ended||Date.now())-started)/1000));$('#order-elapsed').textContent=`${Math.floor(secs/60)}m ${String(secs%60).padStart(2,'0')}s ${ended?'total':'elapsed'}`;}
  function loadClock(hash,isNew){txHash=hash;let saved;try{saved=JSON.parse(localStorage.getItem(clockKey()));}catch{}started=Number.isFinite(saved?.started)&&saved.started>0?saved.started:(isNew?Date.now():0);ended=Number.isFinite(saved?.ended)&&saved.ended>=started?saved.ended:0;terminal=saved?.terminal||'';if(started)saveClock();renderClock();}
  function finishClock(kind,timestamp){if(Number.isFinite(timestamp)&&timestamp*1000>=started)ended=timestamp*1000;else if(terminal!==kind||!ended)ended=Date.now();terminal=kind;saveClock();renderClock();}
  ...
  const timer=setInterval(()=>{if(!complete)renderClock();},1000);
  ...
  return {
    ...
    cancelled(){go(2);},
    sent(hash,isNew=true){loadClock(hash,isNew);go(3);$('#order-reference').textContent='Order '+hash.slice(0,10)+'…'+hash.slice(-6);progress(null,'Payment sent. Waiting for confirmation.','The wallet transaction has been submitted. This is your new order, not a saved example.');link('Your Sepolia payment','https://eth-sepolia.blockscout.com/tx/'+hash);},
    resume(hash){this.sent(hash,false);$('#order-explanation').textContent='Resuming your existing order with its original timer. No new payment is being made.';},
    fundedAt(timestamp){if(!started&&Number.isFinite(timestamp)&&timestamp>0){started=timestamp*1000;saveClock();renderClock();}},
